...
脚本自动：递归扫描目录 → 校验独立目录/目录名唯一 → 跳过已翻译视频 → 所有视频单次流水线完成。

---

## 示例效果

执行翻译命令后，每个视频旁边会生成翻译结果和中间文件：
...
| 类型 | 位置 | 来源 | 用途 |
|------|------|------|------|
| **输入字幕** | 视频/音频旁的同名文件（如 `1.srt`） | 用户提供（可选） | 与 ASR 识别结果交叉校准，提高识别与断句准确性（见[字幕ASR综合校准说明.md](字幕ASR综合校准说明.md)） |
| **ASR 字幕** | `segments/ASR/full_text.srt` | 系统生成 | 原语言全文识别结果，用于检查识别质量；可编辑后 `--edit-rerun` 回写，只重跑受影响部分 |
| **翻译字幕** | `segments/{语言}/full_translation.srt`，每种目标语言一份（如 `segments/English/full_translation.srt`） | 系统生成 | 翻译完成后自动生成（无论是否启用 `--stop-after-translation`），用于人工检查和返修；可编辑后 `--edit-rerun` 回写，只重跑受影响部分 |

三者的关系：**输入字幕**（如有）参与校准生成 **ASR 字幕**，ASR 字幕经翻译生成各语言的 **翻译字幕**。后两类都是人工检查和返修的入口——编辑后通过 `--edit-rerun` 只重跑受影响的段落，无需重新执行整个流程（详见 [客户端编辑重跑模式](#客户端编辑重跑模式)）。
...
> 💡 可运行 `Tools/organize_videos_into_folders.py` 自动整理视频到独立目录。
...
提供两种模式：**基本模式**（单人场景）和**高级模式**（多人/复杂场景）。两者均默认启用 TTS 时长感知翻译，区别在于 ASR 切分精度和非语言检测。
...
预置设置（与高级模式的差异）：
- ASR 模式：basic（ASR 自带说话人切分，不做二次精细切分）
...
### 批量短剧翻译 `batch_video_translate.py`
...
**Q: 任务失败显示错误代码怎么办？**  
A: 错误代码是流水线失败时的简要分类，帮助您快速定位问题。完整错误代码列表及处理建议见 [错误代码说明.md](错误代码说明.md)。所有错误退出后都可以重跑命令续跑。

**Q: 如何强制从零开始？**  
A: 删除视频目录下的 `.vt_task_id` 文件，或运行时加 `--new-task` 参数。

**Q: 视频不在独立目录怎么办？**  
A: 运行 `Tools/organize_videos_into_folders.py` 自动整理。
