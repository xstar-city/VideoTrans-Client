...
启动后，客户端会逐一对比本地与服务端的文件内容/存在性，检测到以下五种编辑后自动处理：

| 场景 | 操作方式 | 客户端检测 | 自动执行 |
|------|----------|-----------|---------|
| **改 ASR 文本** | 编辑 `segments/ASR/{stem}.txt` | 下载服务端 txt 逐字对比，内容不一致 | 上传新 txt；删除所有语言目录下同 stem 的 txt + mp3 + md |
| **改翻译文本** | 编辑 `segments/{lang}/{stem}.txt` | 下载服务端 txt 逐字对比，内容不一致 | 上传新 txt；删除该语言目录下同 stem 的 mp3 + md |
| **删语种** | 删除本地语言目录（如 `English/`） | 本地目录不存在 | 删除服务端对应语言目录 |
| **删某句 mp3** | 删除 `segments/{lang}/{stem}.mp3` | 本地文件缺失 | 删除服务端对应 mp3 |
| **删某句 txt** | 删除 `segments/{lang}/{stem}.txt` | 本地文件缺失 | 删除服务端对应 txt + mp3 + md + 候选目录 |
